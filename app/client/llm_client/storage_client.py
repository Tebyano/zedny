...
class StorageClient:
    def __init__(self, use_service_role: bool = True, bucket: str | None = None):
        ...
        self.bucket = bucket or os.getenv("SUPABASE_STORAGE_BUCKET") or os.getenv("SUPABASE_BUCKET")
        if not self.bucket:
            raise RuntimeError("Missing SUPABASE_STORAGE_BUCKET (or SUPABASE_BUCKET) in environment")
